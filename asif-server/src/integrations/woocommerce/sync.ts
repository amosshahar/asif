...
export async function upsertWcOrderMapped(
  ...
  if (existing.status === 'completed' || hasPickProgress(existing)) {
    await persistence.put({
      ...
      wcDateCreated: mapped.wcDateCreated ?? existing.wcDateCreated,
      shippingCity: mapped.shippingCity ?? existing.shippingCity,
      shippingStreet: mapped.shippingStreet ?? existing.shippingStreet,
