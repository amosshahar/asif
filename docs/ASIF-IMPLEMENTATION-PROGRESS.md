...
| A.5 Persist synced orders | `x` | Firestore `asif_orders` **or** JSON file if no `FIREBASE_*` |

### Phase B — Order model & statuses

| Step | Status | Notes |
|------|--------|--------|
| B.1 Internal status enum + WC mapping | `~` | Added **`queued`** for WC imports; `waiting_cs` not yet |
| B.2 `waiting_cs` when any line missing | `—` | |

### Phase C — Admin orders UI

| Step | Status | Notes |
|------|--------|--------|
| C.1 Orders table + filters | `—` | |
| C.2 Assign / reassign pickers | `~` | **API:** `POST /admin/orders/:id/assign` `{ "collectorId": "001" }` — UI still missing |
...
4. **Run sync:** `POST /admin/woocommerce/sync` after server is up.
...
| Method | Path | Purpose |
|--------|------|---------|
| GET | `/admin/woocommerce/status` | WC configured?, store origin, persistence (`firestore` / `file`), `ordersCollection` |
...
| POST | `/admin/woocommerce/sync` | Pull WC → `asif_orders` or JSON |
| GET | `/admin/orders` | All stored orders |
| POST | `/admin/orders/:id/assign` | `{ "collectorId": "001" }` → `assigned` |

*Admin routes are **unauthenticated** until the **auth** milestone.*
